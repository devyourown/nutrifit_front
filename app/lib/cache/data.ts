...
export async function saveCart(id: string, cart: Cart) {
    await redisClient.hSet("cart", id, JSON.stringify(cart));
}

export async function getCart(id: string) {
    try {
        const cart = await redisClient.hGet("cart", id);
...
export async function saveCheckout(id: string, checkout: Checkout) {
    await redisClient.hSet("checkout", id, JSON.stringify(checkout));
}

export async function getCheckout(id: string) {
    try {
        const checkout = await redisClient.hGet("checkout", id);
